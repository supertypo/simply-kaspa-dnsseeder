...
`/api/metrics` needs to surface crawler and dns counters, but the `web` crate must not depend on `crawler` or `dns`. The bridge is `web::MetricsSource { fn extra(&self) -> SubsystemMap; }` (where `SubsystemMap = BTreeMap<String, serde_json::Value>`), implemented in `dnsseeder::metrics_source::SubsystemMetrics` and injected via `AppState::builder(...).metrics_source(...)`. Add new subsystem metrics there, not in the web crate.
