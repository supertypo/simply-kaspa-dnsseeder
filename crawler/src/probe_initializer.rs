...
#[cfg(test)]
mod drain_tests {
    use super::{KaspadMessagePayloadType, drain_payload_types};
    use std::collections::HashSet;

    /// Every variant of `KaspadMessagePayloadType` defined in the `rusty-kaspa`
    /// version we depend on. If this list ever diverges from the upstream
    /// enum, the `drain_covers_every_non_handshake_payload_type` test below
    /// fails and forces us to update `drain_payload_types`.
